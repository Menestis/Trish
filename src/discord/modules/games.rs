...
pub async fn reaction(ctx: &Context, reaction: &Reaction, added: bool, data: &BotData) -> Result<()> {
    ...
    if reaction.emoji.as_data().as_str() == "✅" && c.confirmation_channel == reaction.channel_id {
        ...
        let everyone_role = match guild.to_guild_cached(ctx).await {
        ...
        let announcement_msg = c.publish_channel.send_message(ctx, |cm| {
            ...
            cm.embed(|ce| {
                ...
                ce.description(format!("**Date: ** {}", dat.get("Date").unwrap()));
        ...
        if added {
            ctx.http.add_member_role(guild.0, user.id.0, game.role.0).await?;
        } else {
            ctx.http.remove_member_role(guild.0, user.id.0, game.role.0).await?;
        }
    }
